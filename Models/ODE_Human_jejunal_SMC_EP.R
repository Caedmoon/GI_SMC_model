...
Model <- function(parms){
  derivs <- function(times, y, parms, fixed) {
    with(as.list(c(y, parms, fixed)), {
      ...
      phi_s_LCaL <- 0.005956 * exp(-y["Vm"] / 40)  # Slow inactivation rate (S-24)
      ...
      KOI1 <- parval[23] * exp(parval[1] + parval[2] * y["Vm"])   # (S-80)
      KI1I2 <- parval[24] * exp(parval[3] + parval[4] * y["Vm"])  # (S-81)
      KC3C2 <- parval[25] * exp(parval[5] + parval[6] * y["Vm"])  # (S-82)
      KC2C1 <- parval[26] * exp(parval[7]+ parval[8] * y["Vm"]) 
      KC1O <- parval[27] * exp(parval[9] + parval[10] * y["Vm"])
      KI2I1 <- parval[28] * exp(parval[11] + parval[12] * y["Vm"])
      KC2C3 <- parval[29] * exp(parval[13] + parval[14] * y["Vm"])
      KC1C2 <- parval[30] * exp(parval[15] + parval[16] * y["Vm"])
      KOC1 <- parval[31] * exp(parval[17] + parval[18] * y["Vm"])
      KI1C1 <- parval[32] * exp(parval[19] + parval[20] * y["Vm"])
      KC1I1 <- parval[33] * exp(parval[21] + parval[22] * y["Vm"])
      KI1O <- parval[34] * exp(parval[35] + parval[36] * y["Vm"])
      ...
      # BK Channel
      #Closed
      d[24] <- KC1C0_BK * y["C1_BK"] + KO0C0_BK * y["O0_BK"] - (KC0C1_BK + KC0O0_BK) * y["C0_BK"]  # closed state of BK channel 0
      d[25] <- KC0C1_BK * y["C0_BK"] + KC2C1_BK * y["C2_BK"] + KO1C1_BK * y["O1_BK"] - (KC1C0_BK + KC1O1_BK + KC1C2_BK) * y["C1_BK"]  # closed state of BK channel 1
      d[26] <- KC1C2_BK * y["C1_BK"] + KC3C2_BK * y["C3_BK"] + KO2C2_BK * y["O2_BK"] - (KC2C1_BK + KC2O2_BK + KC2C3_BK) * y["C2_BK"]  # closed state of BK channel 2
      d[27] <- KC2C3_BK * y["C2_BK"] + KC4C3_BK * y["C4_BK"] + KO3C3_BK * y["O3_BK"] - (KC3C2_BK + KC3O3_BK + KC3C4_BK) * y["C3_BK"] # closed state of BK channel 3
      d[28] <- KC3C4_BK * y["C3_BK"] + KO4C4_BK * y["O4_BK"] - (KC4C3_BK + KC4O4_BK) * y["C4_BK"]  # closed state of BK channel 4
      #Open
      d[29] <- KO1O0_BK * y["O1_BK"] + KC0O0_BK * y["C0_BK"] - (KO0O1_BK + KO0C0_BK) * y["O0_BK"]  # Open state of BK channel 0
      d[30] <- KO0O1_BK * y["O0_BK"] + KO2O1_BK * y["O2_BK"] + KC1O1_BK * y["C1_BK"] - (KO1O0_BK + KO1C1_BK + KO1O2_BK) * y["O1_BK"]  # Open state of BK channel 1
      d[31] <- KO1O2_BK * y["O1_BK"] + KO3O2_BK * y["O3_BK"] + KC2O2_BK * y["C2_BK"] - (KO2O1_BK + KO2C2_BK + KO2O3_BK) * y["O2_BK"] # Open state of BK channel 2
      d[32] <- KO2O3_BK * y["O2_BK"] + KO4O3_BK * y["O4_BK"] + KC3O3_BK * y["C3_BK"] - (KO3O2_BK + KO3C3_BK + KO3O4_BK) * y["O3_BK"]  # Open state of BK channel 3
      d[33] <- KO3O4_BK * y["O3_BK"] + KC4O4_BK * y["C4_BK"] - (KO4O3_BK + KO4C4_BK) * y["O4_BK"]  # Open state of BK channel 4
      # Sodium Channel ODE
      ...
      d[36] <- KI1O * y["I1_Na"] + KC1O * y["C1_Na"] - (KOI1 + KOC1) * y["O_Na"]  # Open 
      d[37] <- KOC1 * y["O_Na"] + KI1C1 * y["I1_Na"] + KC2C1 * y["C2_Na"] - (KC1O + KC1C2 + KC1I1) * y["C1_Na"] # Closed state 1
  ...
  initial <- c(
    ...
    IVS_Ca_I_LCaL = 0,
    d_CaT = 0.0202, #maybe should be -70mV
    f_CaT = 1.0,
    x_Kv = 0.2,
    y_Kv = 0.99,
